...
(`docs/verify-contract.sh`) is the executable source of truth for mechanical
checks; prose snippets are examples, written with `rg` or `grep`, and must not
become a second, drifting implementation. Known violations are tracked, one
entry each, in `docs/contract-exceptions.md` (rule 87).
...
Every open exception is an entry in `docs/contract-exceptions.md`, and
closing its deliverable deletes the entry. An exception to this contract
names:
