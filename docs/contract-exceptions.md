...
- **Scope:**
  - **Primitives.** `treasury.transferFunds`, `createForce`, `assignUnit`, `moveUnitToForce`, `placeUnitInCompany`, `holdUnit` and `releaseHull` (`state.zig`) are not reserve-first. Only four commands call `reserveLedger`.
