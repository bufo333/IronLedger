...
- **Scope:**
  - **Primitives.** `treasury.transferFunds`, `createForce`, `toe.assignUnit`, `toe.moveUnitToForce`, `toe.placeUnitInCompany`, `holdUnit` and `releaseHull` (`state.zig`) are not reserve-first. Only four commands call `reserveLedger`.
...
- **Scope, copies:**
  ...
  - tier-upgrade eligibility (`commands.zig:600`, `hq_ops.startTierUpgrade`, `queries.zig:1954`)
