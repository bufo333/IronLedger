...
- ✅ 12.24 Fewer, meaningful events (play feedback: "too many festivals
  and meaningless events"). The weekly deck rolls only half the weeks
  (`weekly_event_chance_bp`), and the pure-flavour decisions — harvest
  festival, press visit, live-fire exercise, employer inspection — became
  automatic log lines. **The inbox rule**: an event that only moves
  fatigue or morale happens on its own; anything that spends XP, breaks
  hulls or costs supplies is a decision — so pirate and night raids
  (sortie/stand-to vs. trust the pickets), convoy losses and interdiction
  (buy, pay smugglers, or ration), caches, intel and live-fire drills
  became choices, while windfalls stay automatic. A test enforces the rule
  over every deck.
