...
registry, layering-record or baseline lines, and a large item lands as the
increments listed, each on `main` before the next. Regression test first,
then the change. The audit deliverables in `docs/audit-response.md` are
folded in: D26 into C10, D27 and D30 into C12, D28 into C7, D29 into C15,
...
- [ ] C15 routing, trucks, beachhead price (rules 20, 22, 24): quote by tonnage that skips full hops, minimises days then cost then HQ id, capped charter fallback; automatic resupply selects its stocked origin through the same tonnage-aware feasible-route quote; one truck-capacity rule per purpose used everywhere and every active support effect uses canonical operational readiness; beachhead distance in one rule with the rounding decided, Map text from it, field-HQ recovery per §9.6.
...
- [ ] C20 documentation and naming (rules 61, 75, 81, 82, 84): module headers link `docs/mekhq-map.md`; field-HQ capability text and source comments match the regional-only structural-repair/training gates; stale contract-pricing comments describe the average-company offer basis; misplaced comment, wrong §9.8 citations, long formula lines, weekly-hour names.
...
above is complete. One item per branch, in order.

- [ ] P1 Brigade HQ: let the outfit's unique seat advance from regional to brigade through a priced, staffed construction project; enforce the one-brigade limit and expose its capacity and facility effects through shared queries and both frontends.
- [ ] P2 Battle armor and artillery: add complete catalogue, acquisition, force-placement, crewing, readiness, maintenance and persistence flows; make both attachable to companies and apply their owned battle effects.
- [ ] P3 Full MekLab and custom variants: complete construction and class A–F refit rules beyond the current weapon/equipment/ammunition mount editor; persist campaign-owned custom chassis and make saved variants buildable, orderable and refittable through both frontends.
