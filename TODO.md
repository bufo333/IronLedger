...
- [ ] Small gaps, part 2: `validate-data` runs on every build, not only `-Ddata` (rule 58); every module names its MekHQ counterpart and links `docs/mekhq-map.md` (rule 61); a test compares the runtime table registry with the executable DDL (rule 50).
