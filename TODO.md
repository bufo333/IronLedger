...
- [ ] D24 errors after a mutation propagate (A3): `runPolicies` and ship-components-home `shipStock` catch only `InsufficientTreasury`; `commands.zig` purchase paths (`placeUnitInCompany`/`moveUnitToForce` `catch return UnknownForce`, `postToHq catch return UnknownHq`) return the real error.
